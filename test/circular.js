...
import { dequal } from '../src/circular';
...
const Circular = suite('circular references');

Circular('self-referencing objects, same shape', () => {
	const hello = { name: 'hello' };
	hello.self = hello;

	const world = { name: 'hello' };
	world.self = world;

	same(hello, world);
});

Circular('self-referencing objects, different values', () => {
	const hello = { name: 'hello' };
	hello.self = hello;
...
Circular('mutually-referencing objects (doubly-linked list)', () => {
	// this is the exact shape reported in lukeed/dequal#19
	const a = {}, b = {};
	a.next = b;
	b.previous = a;
...
Circular('mutually-referencing objects, different values', () => {
	const a = { id: 1 }, b = {};
	a.next = b;
	b.previous = a;
...
Circular('self-referencing arrays', () => {
	const hello = [1, 2];
	hello.push(hello);
	...
	different(hello, other);
});

Circular('circular reference inside a Map value', () => {
	const hello = new Map();
	const helloVal = { owner: hello };
	hello.set('key', helloVal);

	const world = new Map();
	const worldVal = { owner: world };
	world.set('key', worldVal);

	same(hello, world);
});

Circular('circular reference inside a Set value', () => {
	const helloItem = {};
	helloItem.self = helloItem;
	const hello = new Set([helloItem]);

	const worldItem = {};
	worldItem.self = worldItem;
	const world = new Set([worldItem]);

	same(hello, world);
});

Circular('does not treat unrelated shared shapes as cyclic', () => {
	// two separate but structurally-identical subtrees should still
	// compare correctly once the traversal backtracks past them
	const shared = () => ({ a: 1, b: [1, 2, 3] });
	...
	different(
		{ left: shared(), right: shared() },
		{ left: shared(), right: { a: 1, b: [1, 2, 4] } }
	);
});


Circular('same object referenced twice, non-cyclically, is not mistaken for a cycle', () => {
	const shared = { a: 1 };
	...
	// after backtracking out of `left`, comparing `right` against a
	// *different* shape should not be short-circuited by a leftover
	// stack entry from the `left` comparison
	different(
		{ left: shared, right: shared },
		{ left: { a: 1 }, right: { a: 2 } }
	);
